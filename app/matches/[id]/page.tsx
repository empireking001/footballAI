...
export default async function MatchDetailPage({ params }: PageProps) {
  ...
  if (status === 403) return <><FixtureHeader match={match} badge="VIP prediction" /><Container className="flex flex-col gap-6 py-10 sm:py-12"><Card className="mx-auto w-full max-w-2xl border-primary/25 bg-primary/[0.04]"><CardContent className="p-7 text-center sm:p-10"><LockKeyhole className="mx-auto h-7 w-7 text-primary" /><h1 className="mt-4 font-display text-2xl font-bold uppercase tracking-tight">VIP prediction locked</h1><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-muted">This manually entered prediction is reserved for VIP members. Upgrade your account to view the pick and supporting notes.</p><div className="mt-6 flex justify-center"><Button asChild><Link href="/pricing">View VIP access</Link></Button></div></CardContent></Card><MatchIntelligence match={match} context={match.intelligence} /></Container></>;

  return <><FixtureHeader match={match} badge="Prediction not entered" /><Container className="flex flex-col gap-6 py-10 sm:py-12"><Card className="mx-auto w-full max-w-2xl border-border bg-surface/70"><CardContent className="p-7 text-center sm:p-10"><h1 className="font-display text-2xl font-bold uppercase tracking-tight">No manual prediction yet</h1><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-muted">The administrator has not entered a prediction for this fixture yet. The fixture, kickoff time, and live status will continue to update normally.</p><div className="mt-6 flex flex-wrap justify-center gap-3"><Button asChild><Link href="/predictions/today">Today’s fixtures</Link></Button><Button variant="secondary" asChild><Link href="/predictions/week">Next 7 days</Link></Button></div></CardContent></Card><MatchIntelligence match={match} context={match.intelligence} /></Container></>;
}
