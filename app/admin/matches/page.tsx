...
export default function AdminMatchesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      adminList<Match>('matches', { page, limit: 20, status: status === 'all' ? undefined : status }),
  });
  ...
  const columns: Column<Match>[] = [
    ...
    { key: 'kickoffAt', label: 'Kickoff', render: (m) => formatKickoff(m.kickoffAt) },
    {
      key: 'score',
      label: 'Score',
      render: (m) =>
        m.score.homeFullTime !== undefined ? `${m.score.homeFullTime}-${m.score.awayFullTime}` : '—',
    },
    {
      key: 'status',
      label: 'Status',
      render: (m) => <Badge variant={STATUS_VARIANT[m.status] ?? 'default'}>{m.status}</Badge>,
    },
  ];

  return (
    <div>
      <AdminPageHeader title="Matches" subtitle="Matches populate via league fixture sync." />
