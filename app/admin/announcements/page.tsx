...
export default function AdminAnnouncementsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => adminList<Announcement>('announcements', { page, limit: 20 }),
  });

  const columns: Column<Announcement>[] = [
    { key: 'title', label: 'Title' },
    { key: 'type', label: 'Type', render: (a) => <Badge variant={TYPE_VARIANT[a.type]}>{a.type}</Badge> },
    {
      key: 'isActive',
      label: 'Status',
      render: (a) => <Badge variant={a.isActive ? 'live' : 'default'}>{a.isActive ? 'active' : 'inactive'}</Badge>,
    },
  ];
