...
export default function AdminAuditLogsPage() {
  ...
  const columns: Column<AuditLogEntry>[] = [
    ...
    { key: 'action', label: 'Action', render: (l) => <Badge variant={ACTION_VARIANT[l.action] ?? 'default'}>{l.action}</Badge> },
  ...
  return (
    <div>
      <AdminPageHeader title="Audit logs" subtitle="Every mutating admin action, recorded automatically." />
      <DataTable
        columns={columns}
        rows={data?.data ?? []}
        rowKey={(l) => l._id}
