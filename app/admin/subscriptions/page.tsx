...
export default function AdminSubscriptionsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      adminList<AdminSubscription>("subscriptions", { page, limit: 20 }),
  });
  ...
  const columns: Column<AdminSubscription>[] = [
    ...
    { key: "provider", label: "Provider" },
    {
      key: "amount",
      label: "Amount",
      render: (s) => formatCurrency(s.amount, s.currency),
    },
    {
      key: "status",
      label: "Status",
  ...
  return (
    <div>
      <AdminPageHeader
        title="Subscriptions"
        subtitle="Read-only — activated automatically via payment webhooks."
      />
      <DataTable
        columns={columns}
        rows={data?.data ?? []}
